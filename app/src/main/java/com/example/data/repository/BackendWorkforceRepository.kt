...
class BackendWorkforceRepository(
    ...
    override suspend fun proxyClockIn(employeeId: String, latitude: Double?, longitude: Double?): BackendResult<AttendanceShiftDto> {
        ...
        return try {
            ...
            if (!response.isSuccessful || body?.shift == null) {
                ...
                BackendResult.Failure(userFriendlyError(serverErr, response.code(), "Could not record clock-in."))
            } else BackendResult.Success(body.shift)
        } catch (e: IOException) {
            BackendResult.Failure("Network error: ${e.message ?: "unable to reach the server."}", isNetworkError = true)
        }
    ...
    override suspend fun proxyClockOut(employeeId: String, latitude: Double?, longitude: Double?): BackendResult<AttendanceShiftDto> {
        ...
        return try {
            ...
            if (!response.isSuccessful || body?.shift == null) {
                val serverErr = body?.error ?: extractServerErrorMessage(response.errorBody())
                BackendResult.Failure(userFriendlyError(serverErr, response.code(), "Could not record clock-out."))
            } else BackendResult.Success(body.shift)
        } catch (e: IOException) {
            BackendResult.Failure("Network error: ${e.message ?: "unable to reach the server."}", isNetworkError = true)
        }
